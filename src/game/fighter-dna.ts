export type FighterDnaId = "KAIRO" | "SERA" | "VANTA" | "ROOK";
...
export const FIGHTER_DNA: Readonly<Record<FighterDnaId, FighterDna>> = Object.freeze({
  ...
  ROOK: Object.freeze({
    ...
    moveSpeedScale: 0.91,
    stepSpeedScale: 0.88,
    stepCooldownScale: 1.10,
    perfectEvadeBonusTicks: 0,
    interceptDamageScale: 1.04,
    reversalDamageScale: 1.08,
    comboPressureScale: 0.90,
    signature: {
      intercept: "ANCHOR CHECK",
      reversal: "BRACE BREAK",
      flank: "CORNER CRUSH",
      desperation: "GRAVITY HAMMER",
    },
  }),
});
...
export function resolveContextAttack(situation: ContextAttackSituation): ContextAttackChoice {
  ...
  if (situation.interceptOpen) {
    ...
    if (rook) return { moveId: "bodyBlow", beat: dna.signature.intercept, signature: dna.signature.intercept };
  ...
  if (situation.selfHealth <= 24 && situation.defenderHealth <= 34 && situation.distance <= 1.82) {
    if (kairo || vanta || rook) return { moveId: "power", beat: dna.signature.desperation, signature: dna.signature.desperation };
  ...
  if (situation.flankOpen) {
    ...
    if (rook) return { moveId: "bodyBlow", beat: dna.signature.flank, signature: dna.signature.flank };
  ...
  if (situation.defenderNearWall && situation.distance <= 1.62) {
    ...
    if (rook) return { moveId: stage >= 1 ? "throw" : "bodyBlow", beat: "GRAVITY PRESSURE", signature: null };
  ...
  if (situation.defenderAttacking && situation.distance <= 1.48) {
    const beat = vanta ? "NULL COUNTER READ" : rook ? "BRACE COUNTER READ" : "COUNTER READ";
    return { moveId: "counter", beat, signature: null };
  }
