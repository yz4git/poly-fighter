...
function endpointInRoot(visual: FighterVisual, object: THREE.Object3D): THREE.Vector3 {
  return visual.root.worldToLocal(object.getWorldPosition(new THREE.Vector3()));
}

function classifyPose(visual: FighterVisual): V11Pose {
  const lf = endpointInRoot(visual, visual.leftArm.end);
  const rf = endpointInRoot(visual, visual.rightArm.end);
  const lfoot = endpointInRoot(visual, visual.leftLeg.end);
  const rfoot = endpointInRoot(visual, visual.rightLeg.end);
...
function solveArm(visual: FighterVisual, side: -1 | 1, targetLocal: THREE.Vector3, poleLocal: THREE.Vector3): void {
  ...
  const target = rootPoint(visual, targetLocal)
  ...
  solveTwoBoneIK({
    ...
    target,
    pole: rootPoint(visual, poleLocal),
  });
...
function solveSole(visual: FighterVisual, side: FootSide, targetLocal: THREE.Vector3): void {
  ...
  solveTwoBoneIK({
    ...
    target,
    pole: rootPoint(visual, new THREE.Vector3(sign * 0.20, 0.36, 0.10)),
  });
...
function stanceFeet(visual: FighterVisual, depthScale = 1): void {
  ...
  const width = visual.layout.pelvisWidth * 0.48;
  solveSole(visual, "left", new THREE.Vector3(-width, y, -0.135 * depthScale));
  solveSole(visual, "right", new THREE.Vector3(width, y, 0.165 * depthScale));
}

function idlePose(visual: FighterVisual): void {
  const b = visual.rig.bones;
  visual.hips.position.y -= 0.018;
  b.spineLower.rotation.y += 0.085;
  b.spineUpper.rotation.y -= 0.125;
  b.chest.rotation.y += 0.090;
  b.head.rotation.y -= 0.045;
  solveArm(visual, -1, new THREE.Vector3(-0.155, 0.842, 0.155), new THREE.Vector3(-0.275, 0.770, 0.055));
  solveArm(visual, 1, new THREE.Vector3(0.185, 0.565, 0.115), new THREE.Vector3(0.285, 0.650, 0.030));
  stanceFeet(visual, 1);
}

function guardPose(visual: FighterVisual): void {
  const b = visual.rig.bones;
  visual.hips.position.y -= 0.024;
  b.spineLower.rotation.y += 0.060;
  b.spineUpper.rotation.y -= 0.085;
  b.chest.rotation.y += 0.055;
  solveArm(visual, -1, new THREE.Vector3(-0.140, 0.820, 0.170), new THREE.Vector3(-0.255, 0.755, 0.060));
  solveArm(visual, 1, new THREE.Vector3(0.120, 0.790, 0.205), new THREE.Vector3(0.245, 0.725, 0.070));
  stanceFeet(visual, 0.94);
}

function punchPose(visual: FighterVisual): void {
  const lf = endpointInRoot(visual, visual.leftArm.end);
  const rf = endpointInRoot(visual, visual.rightArm.end);
  const punchSide: -1 | 1 = lf.z > rf.z ? -1 : 1;
  const supportSide = (punchSide * -1) as -1 | 1;
  const b = visual.rig.bones;
  b.spineLower.rotation.y += punchSide * -0.045;
  b.spineUpper.rotation.y += punchSide * 0.070;
  b.chest.rotation.y += punchSide * 0.050;
  solveArm(visual, supportSide, new THREE.Vector3(supportSide * 0.125, 0.790, 0.155), new THREE.Vector3(supportSide * 0.245, 0.720, 0.050));
  stanceFeet(visual, 0.86);
}
...
function kickPose(visual: FighterVisual): void {
  ...
  const b = visual.rig.bones;
  b.spineLower.rotation.x -= 0.060;
  b.spineUpper.rotation.x -= 0.040;
  b.chest.rotation.y += kickSide * -0.065;
  solveArm(visual, -1, new THREE.Vector3(-0.120, 0.805, 0.160), new THREE.Vector3(-0.240, 0.735, 0.045));
  solveArm(visual, 1, new THREE.Vector3(0.135, 0.745, 0.110), new THREE.Vector3(0.245, 0.675, 0.020));
}

function applyPose(visual: FighterVisual): void {
  visual.root.updateMatrixWorld(true);
  const pose = classifyPose(visual);
  if (pose === "IDLE") idlePose(visual);
  else if (pose === "GUARD") guardPose(visual);
  else if (pose === "PUNCH") punchPose(visual);
  else kickPose(visual);
  visual.root.userData.v11ReferencePose = pose;
  visual.root.updateMatrixWorld(true);
}
...
export function applyV11ReferencePose(visual: FighterVisual): FighterVisual {
  ...
  anchor.onBeforeRender = () => applyPose(visual);
  visual.root.add(anchor);
  visual.root.userData.v11PoseController = "CONTINUOUS_MESH_REFERENCE_POSE";
  return visual;
}
