...
function solveSole(visual: FighterVisual, side: FootSide, targetLocal: THREE.Vector3): void {
  ...
  solveTwoBoneIK({ root: visual.rig.bones[`${prefix}Thigh`], mid: visual.rig.bones[`${prefix}Shin`], end: visual.rig.bones[`${prefix}Foot`], target, pole: rootPoint(visual, new THREE.Vector3(sign * 0.20, 0.36, sign * 0.12)) });
...
function punchPose(visual: FighterVisual): void {
  ...
  solveArm(visual, supportSide, new THREE.Vector3(supportSide * 0.130, 0.765, 0.080), new THREE.Vector3(supportSide * 0.255, 0.695, -0.015)); stanceFeet(visual, 0.86);
}
